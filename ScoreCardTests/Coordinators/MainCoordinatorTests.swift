...
struct MainCoordinatorTests {
    ...
    @Test func testPopToRootNavigation() {
        ...
        let coordinator = MainCoordinator(modelContext: modelContext)
        coordinator.push(.home)
        coordinator.push(.gameDetail(Game(name: "Test")))
    ...
    @Test func testDeleteGame() {
        ...
        let modelContext = ModelContext(container)
        let coordinator = MainCoordinator(modelContext: modelContext)
        let game1 = Game(name: "Game 1")
        let game2 = Game(name: "Game 2")
